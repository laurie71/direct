...
var RouteError = utils.inherit(error.SourceError, {
    constructor: function(file, routes, routeIndex, msg) {
        var max = routes.length,
            from = Math.min(0, routeIndex - 2),
            to = Math.max(max, routeIndex + 3),
            idx = routeIndex - from,
            rts;
        
        rts = routes.slice(from, to)
            .map(function(route) {
                try { return JSON.stringify(route) }
                catch (e) { return route.toString(); }
            });            
            
        error.SourceError.call(this, file, -1,-1 , msg);
        
        this.sourceOffset = routeIndex - from;
        this.sourceDetail = rts;
        
        this._captureStack(arguments.callee);
    }
});

function badRoute(file, routes, index, msg) {
    // return new RouteError(file, routes, index, 
    //     'Invalid route [' + index + '] (' + msg + ')'
    // );
    var err = new RouteError(file, routes, index, 
        'Invalid route [' + index + '] (' + msg + ')'
    );
    err.routes = routes;
    return err;
...
utils.debug('... '+JSON.stringify(rt));
        ...
        if (! Array.isArray(rt)) {
            return cb(badRoute(file, routes, i, 
                'not an array'));            
        }
        if (rt.length < 3) {
            return cb(badRoute(file, routes, i, 
