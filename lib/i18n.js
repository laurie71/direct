...
i18n.MessageError = utils.inherit(error.ParseError, {
    constructor: function MessageError() { error.ParseError.apply(this, arguments); }
});
...
i18n.MessageSet = utils.inherit(Object, {
    ...
    _parse: function(file, lines) {
        ...
        while (++index < count) {
            ...
            if (split < 0) throw new i18n.MessageError(
                file, index, 0,
                'Invalid I18N message: missing "="');
            if (split == 0) throw new i18n.MessageError(
                file, index, 0,
                'Invalid I18N message: no key before "="');
            if (! line.slice(split+1).trim()) throw new i18n.MessageError(
                file, index, split+1,
                'Invalid I18N message: no message following "="');
            if (key in dict) throw new i18n.MessageError(
                file, index, 0,
